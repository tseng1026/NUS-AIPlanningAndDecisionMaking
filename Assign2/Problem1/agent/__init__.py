...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    Returns:
    val_fn : shape - (|S|), the optimal value function of the MDP
    policy : shape - (|S|),  the optimal policy of the MDP
    '''
    states = rwd_fn.shape[0]
    val_fn = np.zeros((states, ))    
    ...
    '''
    pre = np.zeros((states, 1))
    nxt = np.zeros((states, 1))
    while True:
        pre = nxt
        for s in range(states):
            pdt = matmul(trn_fn[:,s,:], rwd_fn[s,:].T.reshape((states, 1)) + gamma * pre)
            nxt[s] = np.max(pdt)
        
        if (nxt - pre).all() < delta:
            val_fn = pre.reshape((states, ))
            break

    for s in range(states):
        pdt = matmul(trn_fn[:,s,:], rwd_fn[s,:].T.reshape((states, 1)) + gamma * val_fn.reshape((states, 1)))
        policy[s] = np.argmax(pdt)
        
    return val_fn, policy.astype("int")
