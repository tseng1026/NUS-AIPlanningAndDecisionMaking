...
class GeneratePDDL_Stationary :
	...
	def generateInitString(self) :
		...
		for car in self.state.cars:
			initString += " (blocked pt{}pt{})".format(car.position.x, car.position.y)
			initString += " (at pt{}pt{} {})".  format(car.position.x, car.position.y, car.id)
		
		for x in range(self.width):
			for y in range(self.num_lanes):
				if x > 0 and y > 0:
					initString += " (up_next pt{}pt{} pt{}pt{})".format(x, y, x-1, y-1)
				if x > 0 and y < self.num_lanes - 1:
					initString += " (down_next pt{}pt{} pt{}pt{})".format(x, y, x-1, y+1)
				if x > 0:
	...
	def generateGoalString(self) :
		...
		'''
		goalString = "(at pt{}pt{} agent1)".format(self.state.finish_position.x, self.state.finish_position.y)
		return goalString
...
def generateDomainPDDLFile(gen):
	...
	gen.addAction(name = "UP", 
				  ...
				  precondition_string = "(and (at ?from agent1) (up_next ?from ?to) (not (blocked ?to)))", 
				  effect_string = "(and (at ?to agent1) (not (at ?from agent1)))")
	
	gen.addAction(name = "DOWN", 
				  parameters = (("from", "gridcell"), ("to", "gridcell")),
				  precondition_string = "(and (at ?from agent1) (down_next ?from ?to) (not (blocked ?to)))", 
				  effect_string = "(and (at ?to agent1) (not (at ?from agent1)))")

	gen.addAction(name = "FORWARD", 
				  parameters = (("from", "gridcell"), ("to", "gridcell")),
				  precondition_string = "(and (at ?from agent1) (forward_next ?from ?to) (not (blocked ?to)))", 
				  effect_string = "(and (at ?to agent1) (not (at ?from agent1)))")
