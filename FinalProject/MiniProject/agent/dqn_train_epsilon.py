...
SAVE_PATH = "atari_dqn_epsilon.pth.tar"
SAVE_LOOP = 100
...
if __name__ == "__main__":
    ...
    if os.path.exists(SAVE_PATH):
        ...
        model. load_state_dict(torch.load("atari_dqn.pth.tar", map_location=lambda storage, loc: storage))
        target.load_state_dict(torch.load("atari_dqn.pth.tar", map_location=lambda storage, loc: storage))
    target.eval()
